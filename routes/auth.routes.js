...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const user = new User({ loginName, password: hashedPassword, name, surname });
      await user.save();
...
router.post(
  '/login',
  [
    check('loginName', 'Введите логин').exists(),
  ...
  async (req, res) => {
    try {
      ...
      res.json({ token, userId: user.id, loginName: loginName });
